...
class EnemyStealth(Sprite): # Stealth Enemy
    # enemy init
    def __init__(self, game, x, y):
        #self.spawn
    ...
    def is_player_near(self, threshold=100):
    ...
    def update(self):
        self.angle += 1
        self.image = pg.transform.rotate(self.game.stealth_image, self.angle)
        self.x += self.vx * self.game.dt
        self.y += self.vy * self.game.dt
        if self.is_player_near(200):
             # AI gen modified code
            dx = self.game.player.rect.x - self.rect.x  # difference in x-coordinates
            dy = self.game.player.rect.y - self.rect.y  # difference in y-coordinates
            dist = math.hypot(dx, dy)  # distance between player and enemy
            self.image = pg.transform.rotate(self.game.stealth_image, -self.angle)  # rotate the image
            # End of AI code
